...
- [x] `VT` から `HVC` を探す処理が、左バッファの `B` ブロックを通過できるようにする
- [x] 左バッファを使い切ったとき、既存セル列を右端へ丸ごと退避して左バッファを再確保する手順を設計・実装する
...
- [x] 左端超過時に拡張後の左バッファを1ブロック消費し、左移動を再試行する
- [x] 左方向へ初期バッファ以上に移動する統合テストを書く
