...
--    1: R
--  2. 状態・シンボルの符号化
--    state, symbols の列挙順を 1-origin な正数に昇順に割り当てて二進数にした表現を I と O の列で表す。
--  3. 仮想テープの制約
--    今の HI, HO, HB は 1セルが 0, 1, 空白の TM を直接表現する形式である。
--    よって v0 ではターゲットの TM のシンボル集合をこの 3 状態以下に制限する必要がある。拡張は今はしない。
--  4. 不変条件
--    T には仮想ヘッド記号がいつの時点でも丁度 1 個だけある。
--    D は不変である。(UTM のδ関数はどのようなターゲット TM に対しても同じコードで動く)
--    Q と S は作業後に空白に戻すこと。
--
charS :: S -> Char
charS B  = '_'
